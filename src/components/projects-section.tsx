...
export default function Projects({
  ...
  const targetProjects = ["react-twuzzy", "umbrella-records", "diagram-app"];
  const displayProjects =
    projects?.filter((p) => targetProjects.includes(p.name)) || [];
  ...
  return (
    <div className="relative lg:absolute lg:inset-0 z-20 pointer-events-none lg:pointer-events-none overflow-hidden flex lg:justify-end">
      ...
      <div
        className="w-full lg:w-[75vw] h-auto lg:h-full bg-[#0000FF] pointer-events-auto lg:border-l lg:border-white/20 shadow-none lg:shadow-2xl flex flex-col lg:flex-row disable-transform-on-mobile transition-transform duration-[400ms] ease-out"
        style={{
      ...
      >
        ...
        <div className="w-full lg:w-[60%] h-auto lg:h-screen relative lg:overflow-hidden">
          <div
            className="flex flex-col items-center lg:items-start w-full h-auto lg:h-max lg:min-h-screen p-6 lg:p-12 py-12 lg:pt-[10vh] lg:pb-[3vh] space-y-16 lg:space-y-32 disable-transform-on-mobile transition-transform duration-[400ms] ease-out"
            style={{
            ...
            {!isLoading &&
              ...
              displayProjects.map((project) => {
                ...
                return (
                  <div
                    key={project.id}
                    className="group flex flex-col space-y-6 w-full max-w-2xl"
                  >
                    <a
                      href={project.html_url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full h-[50vh] sm:h-[70vh] bg-white/5 hover:bg-white/10 transition-colors border border-white/10 relative overflow-hidden flex items-center justify-center cursor-pointer"
                    >
